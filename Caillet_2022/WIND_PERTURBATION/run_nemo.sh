...
#SBATCH -J run_AMUXL025_S16_WIND47dN_SLP
...
CONFEXE='AMU12r_wind'  # only for nemo.exe

CASE='S16_WIND47dN_SLP' # should not be too long [>15 char.] otherwise, NEMO file names are affected

YEAR0=1958      #- initial year of the long experiment  [ 4 digits ]
MONTH0=03       #- initial month of the long experiment [ 2 digits ]
DAY0=01         #- initial day of the long experiment   [ 2 digits ]

YEAR_MAX=2019   #- stop after $YEAR_MAX is completed
...
STOCKDIR="$SHAREDSCRATCHDIR/jcaillet"  #- restart, output directory

INPUTDIR="/scratch/cnt0021/egi6035/SHARED/njourd/input/nemo_${CONFIG}"  #- input directory

GLOBAL_SIM='GNJ004'  # ORCA simulation used for BDY, runoff and sss restoring
BDYDIR="${INPUTDIR}/BDY_${GLOBAL_SIM}cor"  #- input directory for BDYs
...
TOPO="BedMachineAntarctica-2020-07-15_v02_ICB380"  # to use bathy_meter_${CONFPAR}_${TOPO}.nc
...
NEMOdir="/scratch/cnt0021/egi6035/SHARED/jcaillet/models/nemo_r5151_UKMO_ISF_r5932_nj/NEMOGCM" # NEMO model directory
XIOSdir="/scratch/cnt0021/egi6035/SHARED/jcaillet/models/xios-1.0" # XIOS directory

FORCINGdir='/scratch/cnt0021/egi6035/SHARED/FORCING_SETS/AMUXL025/WIND_LAT47degN_SLOPE/JRA55' # Atmospheric forcing
...
if [ $BYMONTH -eq 1 ]; then
  ln -s -v iodef_monthly.xml iodef.xml
else
...
if [ $IS_BLK_CORE == ".true." ]; then
  ...
  for NAMAT in sn_wndi sn_wndj sn_wndi_whflux sn_wndj_whflux sn_qsr sn_qlw sn_tair sn_humi sn_prec sn_snow sn_tdif
  do
