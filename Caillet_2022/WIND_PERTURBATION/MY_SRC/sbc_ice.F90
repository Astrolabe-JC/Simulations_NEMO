...
# endif
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   taum_ice        !: wind stress over ice                 [kg/m2/s]
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   qla_oce        !: latent heat flux over ocean                 [W/m2]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   qsb_oce        !: sensible heat flux over ocean               [W/m2]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   qlw_oce        !: longwave radiative heat flux over ocean     [W/m2]
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   taum_oce        !: wind stress over ocean                 [kg/m2/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   emp_oce        !: evap - precip over ocean                 [kg/m2/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   taum_oce_whflux !: wind stress (old wind) over ocean for water and heat flux                [kg/m2/s]
#endif
#if defined key_lim3 || defined key_lim2
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   wndm_ice       !: wind speed module at T-point                 [m/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   wndm_whflux_ice       !: wind speed module at T-point for water and heat flux                 [m/s]
#endif
   ...
   !! $Id: sbc_ice.F90 5621 2015-07-21 11:25:36Z mathiot $ 
...
CONTAINS
   ...
   INTEGER FUNCTION sbc_ice_alloc()
      ...
      ALLOCATE( qns_ice (jpi,jpj,jpl) , qsr_ice (jpi,jpj,jpl) ,     &
         ...
         &      utau_ice(jpi,jpj)     , vtau_ice(jpi,jpj)     , wndm_ice(jpi,jpj)     , wndm_whflux_ice(jpi,jpj)     ,  &
         &      fr1_i0  (jpi,jpj)     , fr2_i0  (jpi,jpj)     , taum_ice (jpi,jpj),     &
#if defined key_lim2
         &      a_i(jpi,jpj,jpl)      ,                             &
#endif
#if defined key_lim3
         ...
         &      taum_oce (jpi,jpj)      , taum_oce_whflux (jpi,jpj)  ,  &
#endif
