...
#  include "domzgr_substitute.h90"
   ...
   !! $Id: limsbc.F90 5621 2015-07-21 11:25:36Z mathiot $
...
CONTAINS
   ...
   SUBROUTINE lim_sbc_flx( kt )
      ...
      IF( iom_use('qla_oce') )   CALL iom_put( "qla_oce" , qla_oce(:,:) * pfrld(:,:) )                                   ! latent flux at ocean surface
      IF( iom_use('qsb_oce') )   CALL iom_put( "qsb_oce" , qsb_oce(:,:) * pfrld(:,:) )                                   ! sensible flux at ocean surface
      IF( iom_use('qlw_oce') )   CALL iom_put( "qlw_oce" , qlw_oce(:,:) * pfrld(:,:) )                                   ! longwave flux at ocean surface
      ...
      IF( iom_use('taum_oce' ) )   CALL iom_put( "taum_oce"  , taum_oce(:,:) * pfrld(:,:) )
      IF( iom_use('taum_oce_whflux' ) )   CALL iom_put( "taum_oce_whflux"  , taum_oce_whflux(:,:) *  pfrld(:,:))
      IF( iom_use('taum_ice' ) )   CALL iom_put( "taum_ice"  , taum_ice(:,:) * (1 - pfrld(:,:)))
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            ...
            ! Total heat flux reching the ocean = hfx_out (W.m-2) 
            !---------------------------------------------------
