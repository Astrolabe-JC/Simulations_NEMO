MODULE sbc_oce
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   taum_whflux              !: module of sea surface stress (at T-point)    [N/m2] 
   !! wndm is used onmpute surface gases exchanges in ice-free ocean or
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   wndm              !: wind speed module at T-point (=|U10m-Uoce|)  [m/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   wndm_whflux       !: wind speed module at T-point (=|U10m-Uoce|)  for water and heat flux [m/s]
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: sbc_oce.F90 5624 2015-07-21 18:47:42Z mathiot $
...
CONTAINS
   ...
   INTEGER FUNCTION sbc_oce_alloc()
      ...
      ierr(:) = 0
      !
      ALLOCATE( utau(jpi,jpj) , utau_b(jpi,jpj) , taum(jpi,jpj) , taum_whflux(jpi,jpj) ,    &
         &      vtau(jpi,jpj) , vtau_b(jpi,jpj) , wndm(jpi,jpj) , wndm_whflux(jpi,jpj) , STAT=ierr(1) ) 
         !
