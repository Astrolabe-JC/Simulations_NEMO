...
!!======================================================================
   ...
   INTEGER , PARAMETER ::   jp_wndi_whflux = 3   ! index of 10m wind velocity for heat and water flux (i-component) (m/s)    at T-point
   INTEGER , PARAMETER ::   jp_wndj_whflux = 4   ! index of 10m wind velocity for heat and water flux (j-component) (m/s)    at T-point
   INTEGER , PARAMETER ::   jp_humi = 5           ! index of specific humidity               ( % )
   INTEGER , PARAMETER ::   jp_qsr  = 6           ! index of solar heat                      (W/m2)
   INTEGER , PARAMETER ::   jp_qlw  = 7           ! index of Long wave                       (W/m2)
   INTEGER , PARAMETER ::   jp_tair = 8           ! index of 10m air temperature             (Kelvin)
   INTEGER , PARAMETER ::   jp_prec = 9           ! index of total precipitation (rain+snow) (Kg/m2/s)
   INTEGER , PARAMETER ::   jp_snow = 10           ! index of snow (solid prcipitation)       (kg/m2/s)
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: sbcblk_core.F90 5621 2015-07-21 11:25:36Z mathiot $
...
CONTAINS
   ...
   SUBROUTINE sbc_blk_core( kt )
      ...
      TYPE(FLD_N) ::   sn_wndi, sn_wndj, sn_wndi_whflux, sn_wndj_whflux, sn_humi, sn_qsr       ! informations about the fields to be read
      TYPE(FLD_N) ::   sn_qlw , sn_tair, sn_prec, sn_snow      !   "                                 "
      TYPE(FLD_N) ::   sn_tdif                                 !   "                                 "
      NAMELIST/namsbc_core/ cn_dir , ln_taudif, rn_pfac, rn_efac, rn_vfac,  &
         &                  sn_wndi, sn_wndj, sn_wndi_whflux, sn_wndj_whflux, sn_humi  , sn_qsr ,           &
...
902      IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_core in configuration namelist', lwp )
         ...
         slf_i(jp_wndi_whflux) = sn_wndi_whflux   ;   slf_i(jp_wndj_whflux) = sn_wndj_whflux
      ...
      IF( MOD( kt - 1, nn_fsbc ) == 0 )   THEN
         ...
         wndi_whflux_ice(:,:)    = sf(jp_wndi_whflux)%fnow(:,:,1)
         wndj_whflux_ice(:,:)    = sf(jp_wndj_whflux)%fnow(:,:,1)  
      ENDIF
#endif
      !
   ...
   SUBROUTINE blk_oce_core( kt, sf, pst, pu, pv )
      ...
      REAL(wp), DIMENSION(:,:), POINTER ::   zwnd_i_whflux, zwnd_j_whflux  ! wind speed components at T-point for water and heat flux
      ...
      REAL(wp), DIMENSION(:,:), POINTER ::   Cd_whflux                ! transfer coefficient for momentum      (tau)
      REAL(wp), DIMENSION(:,:), POINTER ::   Ch_whflux                ! transfer coefficient for sensible heat (Q_sens)
      REAL(wp), DIMENSION(:,:), POINTER ::   Ce_whflux                ! tansfert coefficient for evaporation   (Q_lat)
      ...
      !
      CALL wrk_alloc( jpi,jpj, zwnd_i, zwnd_j, zwnd_i_whflux, zwnd_j_whflux, zqsatw, zqlw, zqsb, zqla, zevap )
      CALL wrk_alloc( jpi,jpj, Cd, Ch, Ce, Cd_whflux, Ch_whflux, Ce_whflux, zst, zt_zu, zq_zu )
      !
      ...
      CALL wnd_cyc( kt, zwnd_i_whflux, zwnd_j_whflux )    ! add analytical tropical cyclone (Vincent et al. JGR 2012)
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vect. opt.
            ...
            sf(jp_wndi_whflux)%fnow(ji,jj,1) = sf(jp_wndi_whflux)%fnow(ji,jj,1) + zwnd_i_whflux(ji,jj)
            sf(jp_wndj_whflux)%fnow(ji,jj,1) = sf(jp_wndj_whflux)%fnow(ji,jj,1) + zwnd_j_whflux(ji,jj)
         END DO
      END DO
#endif
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vect. opt.
            ...
            zwnd_i_whflux(ji,jj) = (  sf(jp_wndi_whflux)%fnow(ji,jj,1) - rn_vfac * 0.5 * ( pu(ji-1,jj  ) + pu(ji,jj) )  )
            zwnd_j_whflux(ji,jj) = (  sf(jp_wndj_whflux)%fnow(ji,jj,1) - rn_vfac * 0.5 * (pv(ji  ,jj-1) + pv(ji,jj) )  )
         END DO
      END DO
      CALL lbc_lnk( zwnd_i(:,:) , 'T', -1. )
      CALL lbc_lnk( zwnd_j(:,:) , 'T', -1. )
      CALL lbc_lnk( zwnd_i_whflux(:,:) , 'T', -1. )
      CALL lbc_lnk( zwnd_j_whflux(:,:) , 'T', -1. )
      ! ... scalar wind ( = | U10m - U_oce | ) at T-point (masked)
      wndm(:,:) = SQRT(  zwnd_i(:,:) * zwnd_i(:,:)   &
         &             + zwnd_j(:,:) * zwnd_j(:,:)  ) * tmask(:,:,1)
      wndm_whflux(:,:) = SQRT(  zwnd_i_whflux(:,:) * zwnd_i_whflux(:,:)   &
         &             + zwnd_j_whflux(:,:) * zwnd_j_whflux(:,:)  ) * tmask(:,:,1)
      ...
      CALL turb_core_2z( rn_zqt, rn_zu, zst, sf(jp_tair)%fnow, zqsatw, sf(jp_humi)%fnow, wndm_whflux,   &
         &               Cd_whflux, Ch_whflux, Ce_whflux, zt_zu, zq_zu )
      !!!A SUPPRIMER
      ! ... tau module, i and j component
      DO jj = 1, jpj
         DO ji = 1, jpi
            zztmp = rhoa * wndm_whflux(ji,jj) * Cd_whflux(ji,jj)
            taum_whflux(ji,jj) = zztmp * wndm_whflux(ji,jj)
         END DO
      END DO  

      ! ... add the HF tau contribution to the wind stress module?
      IF( lhftau ) THEN
         taum_whflux(:,:) = taum_whflux(:,:) + sf(jp_tdif)%fnow(:,:,1)
      ENDIF
      CALL iom_put( "taum_oce_whflux", taum_whflux )   ! output wind stress module
      !!!
      ...
      IF( ABS( rn_zu - rn_zqt) < 0.01_wp ) THEN
         ...
         zevap(:,:) = rn_efac*MAX( 0._wp,     rhoa*Ce_whflux(:,:)*( zqsatw(:,:) - sf(jp_humi)%fnow(:,:,1) )*wndm_whflux(:,:) ) ! Evaporation
         zqsb (:,:) =                     cpa*rhoa*Ch_whflux(:,:)*( zst   (:,:) - sf(jp_tair)%fnow(:,:,1) )*wndm_whflux(:,:)   ! Sensible Heat
      ELSE
         ...
         zevap(:,:) = rn_efac*MAX( 0._wp,     rhoa*Ce_whflux(:,:)*( zqsatw(:,:) - zq_zu(:,:) )*wndm_whflux(:,:) )   ! Evaporation
         zqsb (:,:) =                     cpa*rhoa*Ch_whflux(:,:)*( zst   (:,:) - zt_zu(:,:) )*wndm_whflux(:,:)     ! Sensible Heat
      ENDIF
      zqla (:,:) = Lv * zevap(:,:)

      IF(ln_ctl) THEN
         !CALL prt_ctl( tab2d_1=zqla  , clinfo1=' blk_oce_core: zqla   : ', tab2d_2=Ce , clinfo2=' Ce  : ')
         CALL prt_ctl( tab2d_1=zqla  , clinfo1=' blk_oce_core: zqla   : ', tab2d_2=Ce_whflux , clinfo2=' Ce_whflux  : ' )
         !CALL prt_ctl( tab2d_1=zqsb  , clinfo1=' blk_oce_core: zqsb   : ', tab2d_2=Ch , clinfo2=' Ch  : ')
         CALL prt_ctl( tab2d_1=zqsb  , clinfo1=' blk_oce_core: zqsb   : ', tab2d_2=Ch_whflux , clinfo2=' Ch_whflux  : ' )
         ...
         CALL prt_ctl( tab2d_1=wndm_whflux  , clinfo1=' blk_oce_core: wndm_whflux   : ') 
      ...
      ENDIF
      !
      CALL wrk_dealloc( jpi,jpj, zwnd_i, zwnd_j, zwnd_i_whflux, zwnd_j_whflux, zqsatw, zqlw, zqsb, zqla, zevap )
      CALL wrk_dealloc( jpi,jpj, Cd, Ch, Ce, Cd_whflux, Ch_whflux, Ce_whflux, zst, zt_zu, zq_zu )
      !
   ...
   SUBROUTINE blk_ice_core_tau
      ...
      REAL(wp) ::             zwndi_t_whflux , zwndj_t_whflux               ! relative wind components at T-point for heat and water flux
      !!---------------------------------------------------------------------
      !
...
!!gm end
      ...
      CASE( 'I' )                  ! B-grid ice dynamics :   I-point (i.e. F-point with sea-ice indexation)
         ...
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! B grid : NO vector opt
               ...
               taum_ice(ji,jj) = zcoef_wnorm * wndm_ice(ji,jj) * wndm_ice(ji,jj)
               zwndi_t_whflux = sf(jp_wndi_whflux)%fnow(ji,jj,1) - rn_vfac * 0.25 * ( u_ice(ji,jj+1) + u_ice(ji+1,jj+1)   &
                  &                                                    + u_ice(ji,jj  ) + u_ice(ji+1,jj  )  )
               zwndj_t_whflux = sf(jp_wndj_whflux)%fnow(ji,jj,1) - rn_vfac * 0.25 * ( v_ice(ji,jj+1) + v_ice(ji+1,jj+1)   &
                  &                                                    + v_ice(ji,jj  ) + v_ice(ji+1,jj  )  )
               wndm_whflux_ice(ji,jj)  = SQRT( zwndi_t_whflux * zwndi_t_whflux + zwndj_t_whflux * zwndj_t_whflux ) * tmask(ji,jj,1)  
            END DO
         END DO
         CALL lbc_lnk( utau_ice, 'I', -1. )
         CALL lbc_lnk( vtau_ice, 'I', -1. )
         CALL lbc_lnk( wndm_ice, 'T',  1. )
         CALL lbc_lnk( wndm_whflux_ice, 'T',  1. )
         CALL lbc_lnk( taum_ice, 'T',  1. )
         !
      CASE( 'C' )                  ! C-grid ice dynamics :   U & V-points (same as ocean)
         DO jj = 2, jpj
            DO ji = fs_2, jpi   ! vect. opt.
               ...
               wndm_ice(ji,jj) = SQRT( zwndi_t * zwndi_t + zwndj_t * zwndj_t ) * tmask(ji,jj,1)
               taum_ice(ji,jj)=  zcoef_wnorm * wndm_ice(ji,jj) * wndm_ice(ji,jj) 
               zwndi_t_whflux = (  sf(jp_wndi_whflux)%fnow(ji,jj,1) - rn_vfac * 0.5 * ( u_ice(ji-1,jj  ) + u_ice(ji,jj) )  )
               zwndj_t_whflux = (  sf(jp_wndj_whflux)%fnow(ji,jj,1) - rn_vfac * 0.5 * ( v_ice(ji  ,jj-1) + v_ice(ji,jj) )  )
               wndm_whflux_ice(ji,jj)  = SQRT( zwndi_t_whflux * zwndi_t_whflux + zwndj_t_whflux * zwndj_t_whflux ) * tmask(ji,jj,1)
            END DO
         END DO
         DO jj = 2, jpjm1
      ...
      ! ... add the HF tau contribution to the wind stress module?
      IF( lhftau ) THEN
         taum_ice(:,:) = taum_ice(:,:) + sf(jp_tdif)%fnow(:,:,1)
      ENDIF
      CALL iom_put( "taum_ice", taum_ice )   ! output wind stress module
      ...
      IF(ln_ctl) THEN
         ...
         CALL prt_ctl(tab2d_1=taum_ice  , clinfo1=' blk_ice_core: taum_ice : ')
         CALL prt_ctl(tab2d_1=wndm_whflux_ice  , clinfo1=' blk_ice_core: wndm_whflux_ice : ')
      ENDIF
   ...
   SUBROUTINE blk_ice_core_flx( ptsu, palb )
      ...
      DO jl = 1, jpl                        !  Loop over ice categories  !
         ...
         DO jj = 1 , jpj
            DO ji = 1, jpi
               ...
               ! Sensible Heat
               z_qsb(ji,jj,jl) = rhoa * cpa * Cice * wndm_whflux_ice(ji,jj) * ( ptsu(ji,jj,jl) - sf(jp_tair)%fnow(ji,jj,1) )
               ! Latent Heat
               qla_ice(ji,jj,jl) = rn_efac * MAX( 0.e0, rhoa * Ls  * Cice * wndm_whflux_ice(ji,jj)   &                           
              ...
              ! Latent heat sensitivity for ice (Dqla/Dt)
               IF( qla_ice(ji,jj,jl) > 0._wp ) THEN
                  dqla_ice(ji,jj,jl) = rn_efac * zcoef_dqla * wndm_whflux_ice(ji,jj) / ( zst2 ) * EXP( -5897.8 / ptsu(ji,jj,jl) )
               ELSE
                  dqla_ice(ji,jj,jl) = 0._wp
               ENDIF

               ! Sensible heat sensitivity (Dqsb_ice/Dtn_ice)
               z_dqsb(ji,jj,jl) = zcoef_dqsb * wndm_whflux_ice(ji,jj)
      ...
      ! AJOUT SORTIE
      CALL iom_put( "qlw_ice" , SUM( a_i_b(:,:,:) * z_qlw(:,:,:), dim=3 ) )
      CALL iom_put( "qsb_ice" , - SUM( a_i_b(:,:,:) * z_qsb(:,:,:), dim=3 ) )
      CALL iom_put( "qla_ice" , - SUM( a_i_b(:,:,:) * qla_ice(:,:,:), dim=3 ) )
      CALL iom_put( "emp_ice",   emp_ice )
      CALL iom_put( "qns_tot" ,   qns_tot  )
      CALL iom_put( "qemp_ice" ,   qemp_ice  )
      CALL iom_put( "emp_tot"  ,   emp_tot )
      CALL iom_put( "qsr_tot" ,   qsr_tot  )
      CALL iom_put( "emp_oce" ,   emp_oce  )
      CALL iom_put( "qemp_oce" ,   qemp_oce  )
